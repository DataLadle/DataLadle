...
function TypeBadge({ type }: { type: Gateway["type"] }) {
  ...
  const labels = { Ethernet: "Eth", "Cellular 4G": "4G", WiFi: "WiFi" };
  return (
    <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${styles[type]}`}>
      {labels[type]}
    </span>
  );
}
...
function SignalCell({ gateway }: { gateway: Gateway }) {
  if (gateway.type === "Ethernet") return <span className="text-slate-500">—</span>;
  if (gateway.status === "Offline" || gateway.signalStrength == null) return <span className="text-slate-500">—</span>;
  const dbm = typeof gateway.signalStrength === "number" && gateway.signalStrength <= 100
    ? signalStrengthToDbm(gateway.signalStrength)
...
export default function GatewaysPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950">
        ...
        <main className="p-8">
          <div className="mb-8 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-slate-400">IoT hubs and network management</p>
            </div>
            <button
              type="button"
              ...
              <table className="w-full min-w-[800px]">
                <thead>
                  <tr className="border-b border-slate-700/80">
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Gateway ID</th>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Sensors</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Last Heartbeat</th>
                  </tr>
                </thead>
                <tbody>
                  {MOCK_GATEWAYS.map((gateway) => (
                    ...
                    >
                      <td className="px-4 py-3"><StatusIcon status={gateway.status} /></td>
                      <td className="px-4 py-3"><p className="font-semibold text-white">{gateway.name}</p></td>
                      <td className="px-4 py-3"><span className="font-mono text-sm text-slate-400">{gateway.physicalId}</span></td>
                      <td className="px-4 py-3">
                        <Link href={`/locations/${gateway.locationId}`} className="text-sm text-slate-300 hover:text-[#26ADE4]" onClick={(e) => e.stopPropagation()}>
                          {getLocationById(gateway.locationId)?.name ?? "—"}
                        </Link>
                      </td>
                      <td className="px-4 py-3"><TypeBadge type={gateway.type} /></td>
                      <td className="px-4 py-3"><SignalCell gateway={gateway} /></td>
                      <td className="px-4 py-3"><span className="text-sm text-slate-300">{gateway.sensorCount} Linked</span></td>
                      <td className="px-4 py-3"><span className="text-sm text-slate-500">{formatLastHeartbeat(gateway.lastSeen)}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {showRegisterModal && (
        <RegisterGatewayModal onClose={() => setShowRegisterModal(false)} onRegister={() => setShowRegisterModal(false)} />
      )}
      ...
      {selectedGateway && (
        <GatewayDetailsSlideout
          ...
          onReboot={() => {}}
          onUnclaim={() => setSelectedGateway(null)}
        />
      )}
    </div>
  );

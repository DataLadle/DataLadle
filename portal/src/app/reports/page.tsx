...
import { MOCK_SENSORS, MOCK_BUILDINGS, MOCK_ALERTS } from "@/utils/mockData";
...
function generateReport(
  ...
  const building = MOCK_BUILDINGS.find((b) => b.id === locationId);
  const locationName = building?.name ?? "All Locations";
  ...
  if (type === "compliance") {
    const sensors = locationId === "all"
      ? MOCK_SENSORS.filter((s) => s.type === "Temperature" || s.type === "Humidity")
      : MOCK_SENSORS.filter((s) => s.buildingId === locationId && (s.type === "Temperature" || s.type === "Humidity"));
    ...
    return {
      ...
      rows: sensors.map((s) => {
        ...
        const unit = s.unit || (s.type === "Temperature" ? "°F" : "%");
        return {
          Sensor: s.name,
          Location: s.location,
          Min: hasNum ? `${min}${unit}` : "—",
  ...
  if (type === "health") {
    const sensors = locationId === "all"
      ? MOCK_SENSORS
      : MOCK_SENSORS.filter((s) => s.buildingId === locationId);
    ...
    return {
      ...
      rows: sensors.map((s) => ({
        ...
        "Signal (dBm)": s.signalStrengthDbm,
        Status: s.status,
      })),
    };
  }
...
export default function ReportsPage() {
  ...
  const handleGenerate = () => {
    const data = generateReport(reportType, locationId, dateRange);
    setReportData(data);
  };
